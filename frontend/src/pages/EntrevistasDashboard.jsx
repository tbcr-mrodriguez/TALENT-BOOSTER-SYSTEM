...
const EntrevistasDashboard = () => {
  ...
  // const [cargandoReporte, setCargandoReporte] = useState(false); // ❌ ELIMINADA - no se usa
  ...
  // 🔥 NUEVA FUNCIÓN: Cancelar entrevista
  const cancelarEntrevista = async (entrevistaId, candidatoNombre) => {
    if (!window.confirm(`¿Estás seguro de cancelar la entrevista de "${candidatoNombre}"?\n\nEl candidato ya no podrá acceder y podrás crear una nueva entrevista.`)) {
      return;
    }
    
    try {
      const res = await axios.post(`${API_URL}/entrevista/cancelar/${entrevistaId}`);
      if (res.data.success) {
        alert('✅ Entrevista cancelada exitosamente');
        cargarEntrevistas(); // Recargar la lista
      } else {
        alert('❌ Error: ' + (res.data.error || 'No se pudo cancelar'));
      }
    } catch (error) {
  ...
  const getStatusBadge = (estado) => {
    const config = {
      ...
      expulsado: { bg: '#fecaca', text: '#991b1b', label: '🚫 Expulsado' } 
    };
                ...
                <tr key={e.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '12px' }}>
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      ...
                      {/* 🔥 BOTÓN CANCELAR - Solo para pendiente o en_progreso */}
                      {(e.estado === 'pendiente' || e.estado === 'en_progreso') && (
                        <button 
                          onClick={() => cancelarEntrevista(e.id, e.candidato_nombre)} 
                          style={{ 
                            padding: '6px 14px', 
                            fontSize: '11px', 
                            background: '#ef4444', 
  ...
  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '16px' }} onClick={() => setModalReporteAbierto(false)}>
    <div id="reporte-pdf-content" style={{ background: 'white', maxWidth: '1000px', width: '100%', maxHeight: '90vh', overflow: 'auto', padding: '24px', borderRadius: '24px', position: 'relative' }} onClick={(e) => e.stopPropagation()}>
      ...
      <div style={{ position: 'absolute', top: '16px', right: '16px', display: 'flex', gap: '8px', zIndex: 10 }}>
        <button onClick={exportarPDF} style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#10b981', color: 'white', border: 'none', cursor: 'pointer', fontSize: '16px' }}>📄</button>
        <button onClick={() => setModalReporteAbierto(false)} style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#f1f5f9', border: 'none', cursor: 'pointer', fontSize: '16px' }}>✕</button>
      </div>
