-- =====================================================
-- PROCEDIMIENTOS PARA ENTREVISTAS COMPLETOS
-- =====================================================
...
-- 2. OBTENER ENTREVISTA POR TOKEN (con preguntas)
...
-- 7. OBTENER TODAS LAS ENTREVISTAS (para dashboard)
CREATE OR REPLACE FUNCTION obtener_todas_entrevistas()
RETURNS TABLE(
    id INTEGER,
    candidato_id INTEGER,
    candidato_nombre TEXT,
    candidato_profesion TEXT,
    plantilla TEXT,
    puesto TEXT,
    fecha_inicio TIMESTAMP,
    fecha_fin TIMESTAMP,
...
BEGIN
    ...
    SELECT 
        ...
        e.estado,
        e.puntuacion_global,
        COALESCE((SELECT SUM(r.tiempo_respuesta) FROM respuestas_entrevista r WHERE r.entrevista_id = e.id), 0) as duracion,
        COALESCE((SELECT COUNT(*) FROM respuestas_entrevista r WHERE r.entrevista_id = e.id), 0) as total_respuestas
    FROM entrevistas e
...
-- 8. OBTENER REPORTE COMPLETO DE ENTREVISTA
CREATE OR REPLACE FUNCTION obtener_reporte_entrevista(p_entrevista_id INTEGER)
RETURNS JSONB
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    SELECT jsonb_build_object(
        'entrevista', to_jsonb(e),
        'candidato', to_jsonb(c),
        'plantilla', to_jsonb(p),
        'respuestas', COALESCE((
            SELECT jsonb_agg(jsonb_build_object(
                'pregunta', r.pregunta_texto,
                'respuesta', r.respuesta_texto,
                'tiempo', r.tiempo_respuesta,
                'video', r.respuesta_video_path,
                'analisis', r.analisis_pregunta,
                'puntuacion', r.puntuacion_pregunta
            ))
